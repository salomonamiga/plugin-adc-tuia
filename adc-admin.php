...
class ADC_Admin
{
    ...
    public function enqueue_admin_scripts($hook)
    {
        ...
        $is_settings_page = ($hook === 'toplevel_page_' . $this->plugin_name);
        $is_order_page = (
            $hook === 'adc-videos_page_' . $this->plugin_name . '-order-es' ||
            $hook === 'adc-videos_page_' . $this->plugin_name . '-order-en'
        );

        // Modern admin CSS (extracted from inline styles)
        wp_enqueue_style(
            'adc-admin-css',
            ADC_PLUGIN_URL . 'assets/css/admin.css',
            array(),
            '4.9.0'
        );

        // Main settings page JS (cache, tokens, test connection)
        if ($is_settings_page) {
            wp_enqueue_script(
                'adc-admin-js',
                ADC_PLUGIN_URL . 'assets/js/admin.js',
                array('jquery'),
                '4.9.0',
                true
            );
            wp_localize_script('adc-admin-js', 'adc_admin_config', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('adc_admin_nonce'),
            ));
        }

        // Sortable programs page JS
        if ($is_order_page) {
            wp_enqueue_script('jquery-ui-sortable');
            wp_enqueue_script(
                'adc-admin-order-js',
                ADC_PLUGIN_URL . 'assets/js/admin-order.js',
                array('jquery', 'jquery-ui-sortable'),
                '4.9.0',
                true
            );
            wp_localize_script('adc-admin-order-js', 'adc_admin_config', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('adc_admin_nonce'),
            ));
        }
    ...
    {
        $this->render_url_field('api_url', 'https://api.tutorah.tv/v2', 'URL base de la API (sin slash final).');
    }
    ...
    public function webhook_token_callback()
    {
        ...
        if (empty($token)) {
            echo '<div style="background: #fff3cd; border: 1px solid #ffeeba; padding: 12px; border-radius: 4px; margin-bottom: 15px;">';
            echo '<p style="margin: 0; color: #856404;"><strong>Token no inicializado.</strong> Re-activa el plugin para generar el token automáticamente.</p>';
            echo '</div>';
            return;
        }
