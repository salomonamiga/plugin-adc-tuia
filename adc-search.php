...
class ADC_Search
{
    ...
    private function display_search_results($language = 'es')
    {
        ...
        // Get results with intelligent fallback
        $search_results = $this->get_search_results_with_fallback($search_term, $language);
        $results = $search_results['data'];
        $is_fallback = $search_results['is_fallback'];
        ...
        if (empty($results)) {
            ...
            $output .= $this->render_search_results($search_term, $results, $language, $is_fallback);
        }
    ...
    private function render_search_results($search_term, $results, $language, $is_fallback = false)
    {
        $results_texts = array(
            'es' => array(
                ...
                'results' => 'resultado(s)',
                'fallback_message' => 'No encontramos resultados exactos, pero aquí tienes videos que podrían interesarte:',
            ...
            'en' => array(
                ...
                'found' => 'Found',
                'results' => 'result(s)',
                'fallback_message' => 'No exact results found, but here are videos you might be interested in:',
        ...
        if ($is_fallback) {
            $output = '<h1 class="adc-search-results-title">' . $texts['suggested_videos'] . '</h1>';
            $output .= '<div class="adc-search-fallback-message" style="background: #e8f4fd; padding: 15px; border-left: 4px solid #2196f3; margin: 15px 0; border-radius: 4px;">';
            $output .= '<p style="margin: 0; color: #1976d2;">' . $texts['fallback_message'] . '</p>';
            $output .= '</div>';
        } else {
        ...
        if (count($results) > 6 && !$is_fallback) {
