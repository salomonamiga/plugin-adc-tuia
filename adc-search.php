...
class ADC_Search
{
    ...
    /**
     * Get unified cache duration from admin settings
     */
    ...
    public function ajax_search()
    {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'adc_nonce')) {
    ...
    private function render_video_card($video, $language)
    {
        ...
        // Thumbnail con lazy loading (URL viene de la API)
    ...
    /**
     * Get fallback videos (cache unificado)
     */
    private function get_fallback_videos($language, $limit = 8)
    {
        ...
        try {
            ...
            // Cache with unified duration from admin settings
    ...
    /**
     * Get search results with unified cache duration
     */
    private function get_cached_search_results($search_term, $language)
    {
        ...
        ADC_Utils::debug_log("ADC Search Cache Debug: Original='$search_term', Normalized='$normalized_term', Cache Key='$cache_key'");
    ...
    public function modify_search_query($query)
    {
        ...
        if (!is_admin() && $query->is_main_query() && $query->is_search()) {
            ...
            if (!empty($search_term) && !isset($_GET['adc_search'])) {
                ...
                $redirect_url = add_query_arg('adc_search', $search_term, $redirect_url);
