...
(function ($) {
    ...
    window.ADCVideo = {
        ...
        // Menu Module - Completely rewritten and optimized
        menu: {
            initialized: false,
            observer: null,
            ...
            bindEvents: function() {
                ...
                // Manejar clics en PROGRAMAS (texto + clase CSS)
                ADCVideo.cache.$document.on('click.programs-menu', 'a:contains("PROGRAMAS"), .adc_programs_menu_text, .adc-programs-menu-trigger', function(e) {
                    ...
                    if (!$this.data('programs-configured') || !self.isProperlyConfigured($this)) {
                        ADCVideo.utils.log('Configuring element on click');
                        self.setupProgramElement($this);
                    }
                ...
                // Manejar clics en BUSCADOR (clase CSS)
                ADCVideo.cache.$document.on('click.search-menu', '.adc-search-menu-trigger', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    ADCVideo.utils.log('BUSCADOR click detected');
                    
                    var $this = $(this);
                    
                    // Crear formulario de búsqueda si no existe
                    if (!$this.data('search-configured')) {
                        var $parentLi = $this.closest('li');
                        if (!$parentLi.length) {
                            $parentLi = $this.parent();
                        }
                        
                        var searchHtml = '<div class="adc-menu-search-container">' +
                            '<form class="adc-inline-search-form" action="' + window.location.origin + '/" method="get">' +
                            '<input type="text" name="adc_search" placeholder="Buscar..." class="adc-inline-search-input">' +
                            '<button type="submit" class="adc-inline-search-button">' +
                            '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
                            '<circle cx="11" cy="11" r="8"></circle>' +
                        ...
                        $this.data('search-configured', true);
                        ADCVideo.utils.log('Search form configured');
                    }
                });
