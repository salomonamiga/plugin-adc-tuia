...
class ADC_Video_Display
{
    ...
    {
        // Check for old query-based URLs
        $categoria = get_query_var('categoria');
        $video = get_query_var('video');
        $adc_search = get_query_var('adc_search');
        
        // Also check $_GET for direct access
        if (!$categoria && isset($_GET['categoria'])) {
            $categoria = sanitize_text_field($_GET['categoria']);
        }
        if (!$video && isset($_GET['video'])) {
            $video = sanitize_text_field($_GET['video']);
        }
        if (!$adc_search && isset($_GET['adc_search'])) {
            $adc_search = sanitize_text_field($_GET['adc_search']);
        }
        ...
        if ($adc_search) {
            $search_keyword = $current_language === 'en' ? 'search' : 'buscar';
            $new_url = home_url('/' . ($current_language === 'en' ? 'en/' : '') . $search_keyword . '/' . urlencode($adc_search) . '/');
            wp_redirect($new_url, 301);
            exit;
        }

        // Redirect old program/video URLs
        if ($categoria) {
            $program_keyword = $current_language === 'en' ? 'program' : 'programa';
            $base_url = home_url('/' . ($current_language === 'en' ? 'en/' : '') . $program_keyword . '/' . $categoria . '/');
            
            if ($video) {
                $new_url = $base_url . $video . '/';
            } else {
                $new_url = $base_url;
            }
